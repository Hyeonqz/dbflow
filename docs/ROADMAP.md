...
### 0단계 — 버그픽스 (최우선, S) ✅ 완료
- [x] **CR 상세 에러 표시 버그** — `error && !cr`일 때만 배너 렌더 → 로드 후 에러(승인/적용/롤백 실패, 반려 사유 검증) 전부 안 보임 (`change-requests/[id]/page.tsx:89`). 에러 3종 분리(로드/액션/약화된 통제 알림), 린트 fail-closed(STAGING·PROD), 백업·이력 조회 실패 알림, 롤백 버튼 고착 수정, 네트워크 에러 지역화. 프론트엔드 테스트 인프라(Vitest+RTL) 동반 도입

### 1단계 — 결재 인박스 슬라이스 (전부 S, 기존 테이블 쿼리만으로 가능) ✅ 완료 (E3 제외, 2단계 F1로 이월)
- [x] **E1. "내 결재 대기" 인박스 랜딩** — 대시보드에 내가 결정할 CR 목록(오래된 순) + 내 요청(막힌 지점) 섹션
- [x] **E2. 결재 대기 배지** — 사이드바 숫자 배지("나에게 온 결정"만 카운트) + 브라우저 탭 타이틀 미러링
- [ ] **E3. 재검토 요청 버튼** — 반려 후 수정 → 검토자 decision 리셋 + 재알림 (CR 수정 경로 없음·양쪽 반려 상태 모두 종결·알림 인프라 부재로 2단계 F1로 이월)
- [x] **E8. 위임 중 표시** — 결재자 칩 "위임 중" 배지, 대결자 인박스 라우팅 표면화
