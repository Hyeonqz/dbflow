...
- **직무분리(SoD) — 항상 강제됨**: 한 행위자는 한 CR에서 결재 슬롯을 **최대 1개만** 채울 수 있다(직접+대리, 대리+대리 모두 차단). `approve()` tx 내 가드가 `FOR UPDATE` 하에서 이미 결정/대리한 슬롯을 검사해 2번째 시도를 409로 거부한다. 별도 스펙 `2026-07-21-dbflow-delegation-sod-guard-design.md`로 구현. (구현 전 "OOO 위임의 의도된 동작·후속 결정"으로 두었던 문구를 이 강제 시맨틱으로 대체.)
